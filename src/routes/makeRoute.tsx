...
type RouteBuilder<Params extends z.ZodSchema, Search extends z.ZodSchema> = CoreRouteElements<Params, Search> & {
    (p?: z.input<Params>, search?: z.input<Search>): string;

    useParams: () => z.output<Params>;
    useSearchParams: () => z.output<Search>;
    usePush: () => (params: z.input<Params>, search?: z.input<Search>, options?: PushOptions) => void;
...
export function makeRoute<Params extends z.ZodSchema, Search extends z.ZodSchema = typeof emptySchema>(
    ...
    urlBuilder.useParams = function useParams(): z.output<Params> {
        ...
        if (!res.success) {
            throw new Error(`Invalid route params for route ${info.name}: ${res.error.message}`);
        }
        return res.data;
    };
    ...
    if (info?.search) {
        urlBuilder.useSearchParams = function useSearchParams(): z.output<Search> {
            const res = info.search!.safeParse(convertURLSearchParamsToObject(useNextSearchParams()));
            if (!res.success) {
                throw new Error(`Invalid search params for route ${info.name}: ${res.error.message}`);
            }
            return res.data;
        };
    } else {
...
function convertURLSearchParamsToObject(params: Readonly<URLSearchParams> | null): Record<string, string | string[]> {
    if (!params) {
        return {};
    }
